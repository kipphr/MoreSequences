...
def run_test_draw_points_on_circle():
    ...
    circle = rg.Circle(rg.Point(300, 300), 175)
...
def run_test_pizza():
    ...
    # Test 4:
    title = 'PIZZA test 4:  5 slices, thin (thickness=1) black lines.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.outline_thickness = 3
    circle.fill_color = 'yellow'
    pizza(window, circle, 90, 'black', 1)
    window.close_on_mouse_click()
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for end_point in points:
        cut = rg.Line(circle.center, end_point)
...
def run_test_polygon():
    ...
    # Test 3:
    title = 'POLYGON test 3:  15 segments with thick purple lines'
    window = rg.RoseWindow(600, 600, title)

    circle = rg.Circle(rg.Point(300, 300), 200)
    circle.outline_thickness = 3
    circle.fill_color = 'green'
    polygon(window, circle, 9, 'purple', 10)
    window.continue_on_mouse_click()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(len(points)):
        edge = rg.Line(points[k], points[k-1])
        edge.thickness = thickness
        edge.color = color
        edge.attach_to(window)
    window.render()
...
def run_test_fancy_polygon():
    ...
    circle.fill_color = 'black'
    fancy_polygon(window, circle, 300, 130, 'white', 1)
    window.close_on_mouse_click()
